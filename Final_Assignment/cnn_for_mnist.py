...
from keras.layers import Dense, Activation, LSTM, Dropout, BatchNormalization, Conv2D, Flatten
...
if __name__ == "__main__":
    ...
    classifier.add(Conv2D(32, kernel_size=(3, 3), activation="relu", border_mode="same", input_shape=(1, 28,28)))
    ...
    classifier.compile(optimizer=sgd(lr=0.01, decay=1e-6, momentum=0.9, nesterov=True),
                       ...
                       metrics=['accuracy'])
    #for t in range(3):
    classifier.fit(x=training_images, y=training_labels, epochs=20, batch_size=256)
    classifier.save("saved_models/cnn_mnist_1")#+str(t))
    print("Fit completed")
